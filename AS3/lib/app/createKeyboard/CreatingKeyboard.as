	...
	public class CreatingKeyboard extends RotatableScalable {
		...
		public function CreatingKeyboard(begin:Number, keyAlpha:Number, keyColor, gradAngle:Number, kWidth:Number, kHeight:Number, numKeys:Number, natural:Boolean, outline:Boolean) {
			...
			for (var i:int = begin; i < numKeys; i++) {
				...
				key.addEventListener (TUIOEvent.RollOverEvent, onStartAudio);  // if Finger rolls over key, go to onStartAudio function
				key.addEventListener(TUIOEvent.RollOutEvent, onKeyRelease); // if Finger rolls off key, go to onSpriteRelease function
				key.addEventListener(TUIOEvent.RollOverEvent, onKeyPress);  // if Finger rolls off key, go to onSpritePress function
			}		
